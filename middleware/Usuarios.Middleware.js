  ...
  static async validarActualizacionPerfil(req, res, next) {
    try {
      const schema = Joi.object({
        edad: Joi.number().integer().min(13).max(120).messages({
        ...
        altura_cm: Joi.number().min(50).max(300).messages({
        ...
        peso_kg: Joi.number().min(20).max(500).messages({
        ...
        objetivo: Joi.string().valid("bajar peso", "mantener", "tonificar").messages({
