  ...
  static async validarRegistro(req, res, next) {
    try {
      const schema = Joi.object({
        ...
        nombre_usuario: Joi.string().alphanum().min(3).max(50).required().messages({
        ...
        fecha_nacimiento: Joi.date().max("now").optional().allow(null).messages({
          "date.max": "La fecha de nacimiento no puede ser futura",
        }),

        sexo: Joi.string().valid("masculino", "femenino").optional().allow(null).messages({
          "any.only": 'El sexo debe ser "masculino" o "femenino"',
        }),
      })
